...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
 ...
 // ekfState(6) = ekfState(6) + dtIMU * gyro.z;	// yaw
  Quaternion<float> attitudeEst = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  V3D pqr(gyro.x, gyro.y, gyro.z);
  Quaternion<float> predictedAttitude = attitudeEst.IntegrateBodyRate(pqr, dtIMU);
  V3D predictedRPY = predictedAttitude.ToEulerRPY();
  float predictedRoll = (float)predictedRPY.x;
  float predictedPitch = (float)predictedRPY.y;
  ekfState(6) = (float)predictedRPY.z;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  V3F uBG = attitude.Rotate_BtoI(accel);
  ...
  predictedState(3) = predictedState(3) + uBG.x*dt;
  predictedState(4) = predictedState(4) + uBG.y*dt;
  predictedState(5) = predictedState(5) - 9.81f*dt + uBG.z*dt;
  /////////////////////////////// END STUDENT CODE ////////////////////////////
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  uacc(1) = accel.y;
  uacc(2) = accel.z;
  VectorXf RbgPrimeUaccDt = RbgPrime * uacc * dt;

  gPrime(0, 3) = gPrime(1, 4) = gPrime(2, 5) = dt;
  gPrime(3, 6) = RbgPrimeUaccDt(0);
  gPrime(4, 6) = RbgPrimeUaccDt(1);
  gPrime(5, 6) = RbgPrimeUaccDt(2);
